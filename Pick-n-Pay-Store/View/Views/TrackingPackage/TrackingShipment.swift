        ...
        NavigationView{
            ...
            Divider()
                VStack(spacing: 0){
                    ...
                    TrackProgressView(date: "April 5", status: "Out for delivery",icon: "o.circle.fill", isFirst: false, isLast: false, state: .inProgress  ).previewLayout(.sizeThatFits)
                    TrackProgressView(date: "April 5", status: "Delivered",icon: "d.circle.fill", isFirst: false, isLast: true, state: .upComing  ).previewLayout(.sizeThatFits)
                }
